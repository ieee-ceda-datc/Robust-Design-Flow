...
def gen_scaled_libs(ref_libs:str, lib_directory:str,
    ...
    # separated by space
    power_factors = " ".join([str(x) for x in scaling_factors[3:6]])
    cmd = f"./power_scaling.sh {power_factors}"
    os.system(f"{cmd}")

    const_factors = " ".join([str(x) for x in scaling_factors[6:9]])
    cmd = f"./const_scaling.sh {const_factors}"
    os.system(f"{cmd}")

    pinCap_factors = " ".join([str(x) for x in scaling_factors[9:12]])
    cmd = f"./pinCap_scaling.sh {pinCap_factors}"
    os.system(f"{cmd}")

    # Remove ORG, delay_scaled and intPower_scaled directories
    os.system("rm -rf delay_scaled intPower_scaled const_scaled")
