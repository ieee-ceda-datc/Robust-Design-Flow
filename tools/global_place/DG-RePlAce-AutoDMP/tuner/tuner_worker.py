...
from DGRePlAce import DGRePlAce

from hpbandster.core.worker import Worker
# xxx remove dreamplace
...
class AutoDMPWorker(Worker):
    ...
    def compute(self, config_id, config, budget, working_directory, **kwargs):
        ...
        setup_file = ""
        setup_dir_items = self.log_dir.split("/")
        for i in range(len(setup_dir_items) - 2):
            setup_file += setup_dir_items[i] + "/"
        setup_file = os.getcwd() + "/" + setup_file +  "run_openroad_template.tcl"        
        ...
        work_dir = os.getcwd() + "/" + working_directory
        
        #os.chdir(work_dir)
         
        cmd = "cp " + setup_file + " " + work_dir
        #print("cmd = %s" % (cmd))
        print("cmd = ", cmd)
        os.system(cmd)     
        ...
        new_template_file = work_dir + "/run_openroad_template.tcl"
        self.placer.update_params(params, new_template_file)

        config_filename = opj(working_directory, "parameters.json")
        self.placer.params.dump(config_filename)

        
        print("new_template_file = %s" % (new_template_file))

        result = self.placer.run(work_dir)
